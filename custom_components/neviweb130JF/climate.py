...
def extract_capability_full(cap):
    """Extract swing capability which are True for each HP device and add genereal capability."""
    ...
    def update(self):
        if self._activ:
            ...
            if "error" not in device_data:
                if "errorCode" not in device_data:
                    self._cur_temp_before = self._cur_temp
                    if ATTR_ROOM_TEMPERATURE in device_data and device_data[ATTR_ROOM_TEMPERATURE] is not None and "value" in device_data[ATTR_ROOM_TEMPERATURE]:
                        self._cur_temp = float(device_data[ATTR_ROOM_TEMPERATURE]["value"])
                        self._room_temp_error = device_data[ATTR_ROOM_TEMPERATURE].get("error")
                    else:
                        self._cur_temp = self._cur_temp_before
